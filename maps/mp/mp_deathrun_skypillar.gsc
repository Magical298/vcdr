...
finish()
{
	...
	for(;;)
	{
		...
		if(!isdefined(level.finishtrigger))
			return;
		...
		if(!isdefined(level.mapFinishedFirst))
		{
			level.mapFinishedFirst = true;

			level.snipertrigger delete();
			level.jumptele delete();
		}

		if(isdefined(level.activ) && isdefined(level.actorigin))
		{
...
teleportsniperroom()
{
	...
	for(;;)
	{
		...
		if(isdefined(level.activ) && isdefined(level.snipeactorigin))
		{
...
jumptelegame()
{
	...
	for(;;)
	{
		...
		iPrintLnBold(player.name+ "^1 has entered the jump room");
...
createnuke()
{
trig = getEnt("nukebulletstrigger", "targetname");

trig waittill("trigger", player);

player iprintlnbold("You ^1Evil ^7sorcerer !");
player iprintlnbold("You crafted 250XP! ");
wait 2;
