...
straferunmode()
{
	trig=getent("straferun_trigger","targetname");

	trig waittill("trigger",player);
	iprintlnbold("^1" +player.name +" ^7called air support ! ^1Get yourself some cover !");
	player straferun();
	trig delete();
}
...
jetspawn()
{
	...
	jet_mid = spawn("script_model",(-5231,11369,-1092)); 
	...
	jet_left = spawn("script_model",(-5743,11625,-1092)); 
	...
	jet_right = spawn("script_model",(-4719,11625,-1092)); 
	...
	jet_mid moveTo((-5552,-21364,-436),9);
	jet_left moveTo((-6064,-21108,-436),9);
	jet_right moveTo((-5040,-21108,-436),9);
	...
	thread braxi\_mod::drawinformationsmall( 800, 0.8, 1, "Mirroredge" );
	wait 4;
	thread braxi\_mod::drawinformationsmall( 800, 0.8, 1, "� Wingzor & Paradise" );
}
...
WatchHelicopterTrigger()
{
	...
	while(1)
	{
		...
		iPrintlnBold( "^1>> ^2" + player.name + " has entered the heli" );
		helimodel hide();
		break;
	}
}
...
getaliveJumpers()
{
	...
	{
		if(players[i].pers["team"]=="allies" && players[i].sessionstate=="playing" && !players[i].ghost)
			totaljumpers+=1;
	}
	return totaljumpers;
}
...
scopegame()
{
	level.scopegame=getent("scope_endgame","targetname");
	while(1)
	{
		level.scopegame waittill("trigger",player);
		...
		if(!isdefined(level.actiinRoom))
			level.actiinRoom=false;

		player createEndgame((-2562,-603,-4456),(0,270,0),"m40a3_mp","remington700_mp");

		if(!level.actiinRoom && isdefined(level.activ))
		{
			level.actiinRoom=true;

			level.activ.chopperlink=false;
			level.activ.hardpoint_jugg_goggles destroy();
			level.activ createEndgame((-3562,-2348,-4158),(0,180,0),"m40a3_mp","remington700_mp");
		}
		else if(!level.actiinRoom && !isdefined(level.activ))
		{
			player iprintlnbold("^1No Activator found");
			level.actiinRoom=true;
		}
	}
}

createEndgame(org,ang,weap,weap2)
{
	self setorigin(org);
	...
	{
		self iprintlnbold("Health has been restored");
