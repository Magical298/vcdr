...
main()
{
  addQueueTrigger("minigames");
 	addQueueTrigger("weapons_trig");
	addQueueTrigger("bounce_trig");
	addQueueTrigger("sniper_trig");
	addQueueTrigger("sniper_trig_2");
	addQueueTrigger("shotgun_trig");
  addQueueTrigger("knife_trig");
...
x1 delete();
xt1 delete();
player iPrintLnBold("^1You ^7won ^3400 ^7XP !");
...
countdown1()
{
    ...
    while(1)
    {
        ...
        if(self.timelimit1 < 0)
        {
            ...
            self iPrintLn("You failed secret");
        }
...
trigwait(bag, bagnum)
{
    ...
    level.bag_left--;
    level.found++;
    iPrintLnBold( player.name + " found the bag ^1" + bagnum );
    wait 0.2;
}
...
WatchHelicopterTrigger()
{
	...
	while(1)
	{
		...
		iPrintlnBold( "^1" + level.activ.name + " ^7has entered the ^1heli ^7:]" );
...
{
wait 10;
iprintlnBold("^1Welcome ^7to ^1SuX ^7Lolz Map ^1:^7]");
wait 1;
...
for(;;)
	{
		...
		user GiveWeapon( "m40a3_mp" );
		user GiveMaxAmmo( "m40a3_mp" );
		iPrintLnBold ("^3VIP " + user.name + " ^7is ^2Connected");
		wait 3;
		}
			else
			{
...
trigger = GetEnt( "end", "targetname" );
	...
	trigger waittill ("trigger", user);
		{
		iprintlnbold( "^7 " + user.name + " ^1Finished First^7 !!");
		wait 0.1;
		}
	}
}
...
iPrintlnBold( " ^1" + player.name + " ^7has ^1entered ^7the ^1game ^7selection !" );
player TakeAllWeapons();
player antiglitch();
...
{ 
self common_scripts\utility::waittill_any("death","disconnect");
iPrintlnBold("^1"+self.name+" ^7is ^1dead"); 
iPrintlnBold("^1Selection ^7Room is now ^1open ^7!"); 
}
...
for(;;)
{
	...
	level.activ setorigin(acti.origin);
	level.activ setPlayerAngles(acti.angles);
	level.activ giveweapon("m1014_grip_mp");
	...
	level.activ switchtoweapon("m1014_grip_mp");
	}
}
