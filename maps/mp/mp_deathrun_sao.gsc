...
I see... you really are a noob for decompiling my map..
This proves that you really are a noob and cant do anythink on your own..

HOW ABOUT LEARN TO SCRIPT YOUR SELF?

IF YOU WANT TO USE ANY OF THOSE SCRIPTS ASK FOR [PERMISSON]!
STEAM: 321legend123

Credits: VC'Blade - creator script help
...
main()
{
	...
	/////////////////////
	wait 6;
	iprintln("[^5Aincrad^7] - Traps loaded");
	thread d_trap1();
	thread d_trap2();
	thread d_trap3();
	thread d_trap4();
...
endgame_credits()
{
	...
	{
		iprintlnbold("Thanks On Playing SAO Map by eBc|Legend");
		thread endicon();
	}
}
...
creator_menu_trig()
{
	...
	for(;;)
	{
		...
		if (vip == level.me /*|| vip == level.me1 */|| vip == level.blade)
		{
			...
			player iprintlnbold("^5* * ^1VIP ^3Menu ^2Executed^1! ^5* *");
			...
			if( player.used == 4)
			{
				player iprintlnbold("Are you fucking retarded?! Acces denied what dont you understand?!");
				player suicide();
				wait .1;
				player iprintlnbold("Thats punishment!");
				player.used = 0;
			}
			player iprintlnbold("^1...^7Access Denied^1!...");
			player.used++;
			}
	}
}
...
creator_entered()
{
	...
	for(;;)
	{
		...
		if(connected==level.me && !player.pers["legend"])
		{
			...
			iprintlnbold("^5* * ^2Creator ^3" + player.name + "(eBc|Legend) ^2has Joined^1!^5 * *");
		}
	}
}
...
blade_entered()
{
	for(;;)
	{
		...
		if(connected==level.blade && !player.pers["blade"])
		{
			...
			iprintlnbold("^5* * ^2Helper ^3" + player.name + "(VC'Blade) ^2has Joined^1!^5 * *");
		}
	}
}
...
randomroompick()
{
	...
	if (x == 0)
	{
		...
		iprintlnbold("^0Dungeon^7 Teleport is ready!");
		thread removeglass();
	}
	if (x == 1)
	{
		...
		iprintlnbold("^1Ruby Palace^7 Teleport is ready!");
	...
	if (x == 2)
	{
		...
		iprintlnbold("^3City^7 Teleport is ready!");
		if(isdefined(level.activ))
			thread actitele2();
	}
	thread sao_music();
...
sao_music()
{
	...
	{
		ambientplay("sao1");
		iprintln("^3Music:^7 ^5Sword Art Online - Opening");
	}
	if(x == 1)
	{
		ambientplay("sao2");
		iprintln("^3Music:^7 ^5Sword Art Online II - Opening");
	}
	if (x == 2)
	{
		ambientplay("sao3");
		iprintln("^3Music:^7 ^5Sword Art Online II - 2nd Opening");
	}
}
...
partysong()
{
	...
	case 0:
		...
		ambientstop("sao2");
		ambientstop("sao3");
		iprintlnbold("DJ ^3"+user.name+"^7 started the party!");
		level.music destroy();
		wait .1;
		iprintln("Music: ^5Martin Garrix - Forbiten Voices");
		break;

	case 1:
		ambientplay("illuminati");
		ambientstop("sao1");
		ambientstop("sao2");
		ambientstop("sao3");
		iprintlnbold("DJ ^3"+user.name+"^7 started the party!");
...
c_trap7()
{
	...
	trig waittill("trigger", user );
	{
		...
		//iprintlnbold("^0Change ^7Of ^1Heart Deactivated!");
		level.changeofheart = false;
	}
}
...
sword_dance()
{
    ...
    while(1)
    {
		...
		// =====================================
			...
			player TakeAllWeapons();
			player GiveWeapon( "sao_sword_mp" );
...
suicide_jumper()
{
	...
	{
	self suicide();
	self iprintlnbold("^1Shot failed, room autokill");
	}
}
...
sniper2()
{
	...
	for(;;)
	{
		...
		{
			player iprintlnbold("Sniper Token Already Used!");
		}
		wait .001;
	}
}
