...
dunkroom()
{
	...
	while(1)
	{
		...
		iPrintlnBold( " ^7" + player.name + " ^7has entered ^5D^7unk ^5R^7oom^5!!" );	
                                     while( isAlive( player ) && isDefined( player ) )
			wait 5;
	}
}
...
dunk_jump_trig()
{
	...
	while(1)
	{
		...
		iPrintLnBold("^5A^7ctivator ^5G^7oing ^5D^7own^5!!");
...
weaproom()
{
	...
	while(1)
	{
		...
		iPrintlnBold( " ^7" + player.name + " ^7has entered ^5W^7eapon ^5R^7oom^5!!" );	
		wait 5;
		player FreezeControls(0);
		level.activ FreezeControls(0);
		player iprintlnbold("^5F^7IGHT^5!!");
...
WatchHelicopterTrigger()
{
	...
	while(1)
	{
		...
		iPrintlnBold( "^1>> ^2" + player.name + " has chosen ^1Helicopter War!" );
		wait 2;
		iPrintlnBold( "^1FIGHT!" );
		break;
	}
...
StartHelicopter()
{
	...
	while( isDefined( self ) && isAlive( self ) )
	{
		...
		wait int(path[pathnum].waittime);
		pathnum++;
