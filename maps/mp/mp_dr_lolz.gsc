...
main()
{
 ...
 game["axis_soldiertype"] = "desert";
	...
	thread runwaysetup();
	thread knife();
	thread kniferoomturn();
	thread shotgun();
	thread shotgunactifail();
	thread shotgunjumperfail();
	thread old();
	thread oldfakewall();
	thread tpsetup();
	thread watterFX();
...
while(1)
	{
		...
		wait 4;
		iPrintLn("^5Visit: www.fnrp-servers.com !!");
		wait 4;
...
giverpg()
{
	...
	for(;;)
	{
		...
		player iprintlnbold("GOOD ^5LUCK");
	}
}
...
tphardsecret()
{
	...
	for (;;)
	{
		...
		iprintlnbold(player.name + " ^7entered the ^1HARD ^7Secret !!");
...
tpafterpick = getEnt("origin_end_sanic_secret", "targetname");
	...
	iprintlnbold(player.name + " is ^5Sanic ^7! D:");
...
lolzsecrettp333()
{
...

	{
		...
		wait 1;
		iprintlnbold(player.name + " what are you doin HERE?? :o");
	}
}
