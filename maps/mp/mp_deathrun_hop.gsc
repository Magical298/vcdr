/*FileHead:
	Map created by: Siaara (or Raichu)
	Xfire: hellovin
	Tested by: Siaara (or Raichu), Atggost, Nagyii
	Youtube channel: MetubeGames
	Website: www.metube.atw.hu
	Version: v2.0
...
main()
{
	...
	begin_door()
	{
		...
		wait 1;
		iPrintLnBold("^4Map created by: ^3Siaara (Raichu)");
		wait 1;
	...
	secret_music()
	{
		...
		if ( music_player == 0 )
		{
			...
			if ( music_pointer == 0 && player.point == 1)
			{
				...
				music = musicname[m];
				iPrintLnBold(player.name + " ^1 changed the music!");
				if ( music[m] != "none")
				{
	...
	secret_weapon()
	{
		...
		while(1)
		{
			...
			player giveweapon(weapon_secret);
			player givemaxammo(weapon_secret);
	...
	bad_luck()
	{
		...
		wait 1;
		self iPrintLnBold("^1Sorry... you will die within 10 seconds...");
		wait 10;
		self suicide();
	}
	...
	{	
		...
		while(1)
		{
			...
			if(!who.correct == 1)
			{
				...
				iPrintLnBold(who.name + " ^4is in the secret!");
			}
			wait .1;
		}
	}
