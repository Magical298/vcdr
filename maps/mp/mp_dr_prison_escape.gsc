...
secret_tp(){
	while(1){
		...
		iPrintln(player.name+"^8 has entered the ^9Secret!");
...
begin(){
	...
	iPrintLnBold("^3Map made by: ^1Gabb");
	iPrintln("^1I suggest not to use fullbright for the full experience!");
	iPrintln("^1mp_dr_prison_escape v1.0");
	thread music();
	wait(5);
...
contact(){
	wait 20;
	while(1){
		iprintln("^3Steam: ^1Gabb (Korean Girl Picture)");
		wait 2;
		iprintln("^3BNet: ^1Gabb#2215");
		wait 2;
		iprintln("Or send me a pm on ^1FNRP-Forums");
		wait 20;		
	}
}
...
music(){
	...
	if(randomInt == 0){
		...
		while(level.isFinished == false){
			iprintln("^2Now playing: ^62Pac - Gangsta Party feat. Snoop Dogg");
			wait 25;
		}
	}
	...
	if(randomInt == 1){
		...
		while(level.isFinished == false){
			iprintln("^2Now playing: ^62Pac - Hail Mary");
			wait 25;
		}
	}
	...
	if(randomInt == 2){
		...
		while(level.isFinished == false){
			iprintln("^2Now playing: ^6Eazy-E - Boyz N' Tha Hood");
			wait 25;
		}
	}
	...
	if(randomInt == 3){
		...
		while(level.isFinished == false){
			iprintln("^2Now playing: ^62Pac - California Love feat. Dr.Dre");
			wait 25;
		}
	}
	...
	if(randomInt == 4){
		ambientPlay("gthang");	
		while(level.isFinished == false){
			iprintln("^2Now playing: ^6Dr.Dre - Nuthin' but a G Thang feat. Snoop Dogg");
			wait 25;
		}
	}
	...
	if(randomInt == 5){
		...
		while(level.isFinished == false){
			iprintln("^2Now playing: ^6The Notorious B.I.G. - Hypnotize");
			wait 25;
		}
	}
	...
	if(randomInt == 6){
		ambientPlay("nwa");	
		while(level.isFinished == false){
			iprintln("^2Now playing: ^6N.W.A. - Fuk da Police");
			wait 25;
		}
	}
	...
	if(randomInt == 7){
		ambientPlay("realgs");	
		while(level.isFinished == false){
			iprintln("^2Now playing: ^6Eazy-E - Real Muthafuckin G's");
			wait 25;
		}
	}
	...
	if(randomInt == 8){
		...
		while(level.isFinished == false){
			iprintln("^2Now playing: ^6Dr.Dre - Still D.R.E. feat. Snoop Dogg");
			wait 25;
		}
	}
	...
	if(randomInt == 9){
		ambientPlay("hood");	
		while(level.isFinished == false){
			iprintln("^2Now playing: ^6DMX - Where the hood at?");
			wait 25;
		}
	}
}
...
endroom_template(trigger, jumper_origin, acti_origin, weapon, weapon2, weapon_room, song_name){
    ...
    while(1){
		...
		if(song_name == "demboyz"){
			iPrintLn("^6Now Playing: ^5Wiz Khalifa - We Dem Boyz (Subtronikz Remix)");
		}

		if(song_name == "fire"){
			iPrintln("^6Now Playing: ^5BTS - Fire");
			player thread giveRPGAmmo();
			level.activ thread giveRPGAmmo();
		}

		if(song_name == "panda"){
			iPrintln("^6Now Playing: ^5Desiigner - Panda");			
		}

		if(song_name == "howwedo"){
			iPrintln("^6Now Playing: ^5The Game - How we do feat. 50 Cent");
		}

		if(song_name == "trapqueen"){
			iPrintln("^6Now Playing: ^5Fetty Wap - Trap Queen (Crankdat Remix)");
		}
