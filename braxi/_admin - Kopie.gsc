...
adminCommands( admin, pickingType )
{
	...
	{
		case "donatevcp":
				donator=getPlayer(arg1,pickingtype);
				amount=100;

				if(pickingtype == "number")
					rec = int(admin[2]);//player
				else
					rec = admin[2];

				receiver=getPlayer(rec,pickingtype);

				donator_points = donator getStat(2358);

				if(donator_points >= amount)
				{
					donator setStat(2358, donator_points - amount);

					receiver blade\_credits::claimReward( amount );

					iPrintln("^5Vistic^7 Admin: "+donator.name+" Donated ^5"+amount+" VCP ^7To "+receiver.name);
				}
		...
		case "donatejewels":
				...
				donator_jewels = donator getStat(2392);

				if(donator_jewels >= amount)
				{
					donator setStat(2392, donator_jewels - amount);

					receiver blade\_drop::jewelDrop(amount);

					iPrintln("^5Vistic^7 Admin: "+donator.name+" Donated ^5"+amount+" Jewels ^7To "+receiver.name);
				}
		...
		case "addvcp":
			player=getPlayer(arg1,pickingType);
			player setStat(2358,player getstat(2358)+int(admin[2]) );
			break;
